...
% ====== defaults (R2017 compatible, no 'arguments' block) ======
if nargin < 4 || isempty(scale), scale = []; end
if nargin < 5 || isempty(p), p = 0.05; end
if nargin < 6 || isempty(start_th), start_th = -1; end
if nargin < 7 || isempty(end_th), end_th = -1; end
if nargin < 8 || isempty(events_number_control), events_number_control = 0.1; end
if nargin < 9 || isempty(events_days_control), events_days_control = 120; end
if isa(extreme_type,'string'), extreme_type = char(extreme_type); end
...
for ccrieria = 1: size(criterias,2)
    ...
    if ccrieria==2
        maskNOBEST = cellfun(@(s) isequal(s,'NOBEST'), Best_dist_dr);
        mat_id(maskNOBEST)=nan;
    end
    % initial value
...
%         heatwave_daily0=PRM_extreme_identification( "hw", Date, SHI, start_th_h, end_th_h, best_combination_hw(1), best_combination_hw(2));
        ...
        elseif days_per_yr > events_days_control % if the days per year are already larger than the events_days_control
            ...
            if isequal(best_dist_dr,'gev') && h_dr==0  % if best_dist_dr=="gev" & h_dr==0, then meet the two assumptions
...
%     toc
    ...
    maskOK = false(ID_R,ID_M);
    for rr=1:ID_R
        for mm=1:ID_M
            maskOK(rr,mm) = isequal(Best_dist_dr{rr,mm},'gev') && H_dr(rr,mm)==0;
        end
    end
    [id_r_best,id_m_best] = find( maskOK );

    if ~isempty(id_r_best) % if there is no best using the AIC and RMSD as the criteria, then only go to the AIC
        N0=length(id_r_best);

        events_per_yr = nan(N0,1);
        days_per_yr   = nan(N0,1);
        for i=1:N0
            extreme_daily = PRM_extreme_identification( extreme_type, Date, SPI_or_SHI, start_th,end_th ,...
                REMO_d(id_r_best(i)), MERG_d(id_r_best(i), id_m_best(i)) );
